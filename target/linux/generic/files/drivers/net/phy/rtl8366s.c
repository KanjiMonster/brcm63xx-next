...
static int rtl8366s_sw_get_port_link(struct switch_dev *dev, int port,
				     struct switch_port_state *state)
{
	...
	if (data & RTL8366S_PORT_STATUS_LINK_MASK) {
		state->link = 1;
		state->speed = data & RTL8366S_PORT_STATUS_SPEED_MASK;
		state->duplex = !!(data & RTL8366S_PORT_STATUS_DUPLEX_MASK);
